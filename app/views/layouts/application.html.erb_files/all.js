...
var Account = {
  showOpenId: function() {
    $('username_and_password').hide();
    $('open_id').show();
    $('account_identity_url').focus();
  },
  
  hideOpenId: function() {
    $('open_id').hide();
    $('username_and_password').show();
...
var Calendar = {
  adjustEventRepeatControls: function() {
    var repeat, value;
    if (repeat = $('event_repeat_type').value) {
      $('repeat_for').show();
      value = $('event_repeat_count').value;
      $('repeat_for_units').update(Calendar.unitForRepeatValue(repeat) +
        (value == 1 ? '' : 's'));
    } else {
      $('repeat_for').hide();
    }
...
var Grid = {
  ...
  resizeWeek: function(week, nextWeek) {
    ...
    for (var i = 0; i < cells.length; i++) {
      cell = cells[i];
      if (cell.tagName != 'DIV' || cell.className != 'td') continue;
...
document.observe("dom:loaded", function() {
  var link = $(document.documentElement).down("head").down("link[href*=/all.css]"), domain = "";

  if (link) {
    var url = link.readAttribute("href").match(/:(\/\/.*?)\//);
    if (url) domain = url[1];
  }
  ...
  if (domain || !link) {
    (function() {
      ["/images/dots-white.gif",
       ...
       "/images/nubbin.gif",
       "/images/make_a_new_page-pressed.gif",
...
var SelectAllCheckboxesObserver = Class.create({
  initialize: function(container, aggregator) {
    this.container  = $(container);
...
document.observe("dom:loaded", function() {
  if (document.body.hasClassName("sharing")) {
    $("set_visibility").observe("click", function(event) {
      var radio = event.findElement("input[type=radio]");
      if (radio) {
        if ($F(radio) == "users" && !$("user_checkboxes").visible()) {
          $("user_checkboxes").transition(function() {
            $("user_checkboxes").show();
          });
        } else if ($F(radio) != "users" && $("user_checkboxes").visible()) {
          $("user_checkboxes").transition(function() {
...
var Signup = {
  proposeSubdomain: function() {
    ...
    var subdomain = userName.replace(/[^a-zA-Z0-9]/g, "").toLowerCase();

    if (document.getElementById('account_subdomain').value == '') {
      document.getElementById('account_subdomain').value = subdomain;
    }
  ...
  updateSiteUrl: function() {
    $('site_url_name').innerHTML = $('account_subdomain').value + ".backpackit.com";
  },

  enableSignup: function(checkbox) {
    $$('.signup_submit').each(function(submit) { submit.disabled = !checkbox.checked })
  },
  
  switchToOpenId: function(container) {
    this.showOpenId(container);
    this.clearOpenId(container);
    this.showOpenIdCompletion();
  },
  
  swithToUserNameAndPassword: function(container) {
    this.hideOpenId(container);
    this.clearUserNameAndPassword(container);
    this.hideOpenIdCompletion();
  },
  ...
  clearOpenId: function(container) {
    var identity_url = $(container).getElementsByClassName('identity_url').first();
    identity_url.value = "";
    identity_url.focus();    
  },
  
  clearUserNameAndPassword: function(container) {
    $(container).getElementsByClassName('password').each(function(password) { password.value = "" });
    
    var user_name = $(container).getElementsByClassName('user_name').first();
    user_name.value = "";
    user_name.focus();
  },
  
  showOpenIdCompletion: function() {
    $('open_id_completion').show();
    $('user_name_and_password_completion').hide();
  },

  hideOpenIdCompletion: function() {
    $('open_id_completion').hide();
    $('user_name_and_password_completion').show();
  }
}
...
var Slate = {
  ...
  showFromWidgetBarLink: function(widget, link, callback) {
    // TODO: If a slate form is visible and an add link is clicked from
    // the other widget bar, cancel that widget first, then reposition
    // and show the slate
...
document.observe("keypress", function(event) {
  ...
  if (textarea && event.keyCode == Event.KEY_RETURN) {
...
var Users = {
  showOpenId: function(container) {
    if ($('link_to_open_id')) $('link_to_open_id').hide();
    if ($('link_to_normal_login')) $('link_to_normal_login').show();
    $(container).select('.username_entry').invoke('hide');
    $(container).select('.open_id_entry').invoke('show');
    $(container).select('.identity_url').first().focus();
  },
  
  hideOpenId: function(container) {
    if ($('link_to_open_id')) $('link_to_open_id').show();
    if ($('link_to_normal_login')) $('link_to_normal_login').hide();
    $(container).select('.username_entry').invoke('show');
    $(container).select('.open_id_entry').invoke('hide');
    $(container).select('.identity_url').first().value = "";
    $(container).select('.user_name').first().focus();
  }
}
